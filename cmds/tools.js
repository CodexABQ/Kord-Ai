...
${c} testdb                - check the Mongo connection is working
${c} <id> price <value>
${c} <id> caption <text>
${c} <id> contact <text>
${c} <id> link <url>
...
logged to that item's history.\`\`\``
    ...
    if (first === "testdb") {
      try {
        ...
        const itemCount = await db.collection("marketplace_items").estimatedDocumentCount()
        const catCount = await db.collection("marketplace_categories").estimatedDocumentCount()
        return await m.send(`\`\`\`✓ Mongo connection OK (${ms}ms)\nDB: ${db.databaseName}\nItems stored: ${itemCount}\nCategories: ${catCount}\`\`\``)
      } catch (e) {
        console.log("marketplace testdb error", e)
        return await m.send(`_✘ Mongo connection FAILED: ${e.message}_\n_Check MARKETPLACE_MONGO_URI (and MARKETPLACE_DB_NAME if needed) on Render_`)
      }
    }
    ...
    if (first === "grab") {
      ...
      // ── compress images before upload — HD photos from phone cameras
      //    can be several MB, and Render's free-tier outbound bandwidth
      //    to the CDN is slow enough that large files time out. Videos
      //    are skipped (Jimp is image-only); large videos may still time
      //    out and would need a separate ffmpeg-based fix if that comes up.
      let uploadPath = path
      ...
      if (type === "image") {
        try {
          ...
          compressedPath = `${path}_c.jpg`
          await image.scaleToFit(1280, 1280).quality(65).writeAsync(compressedPath)
          uploadPath = compressedPath
        } catch (e) {
          console.log("marketplace compress error, uploading original", e)
          uploadPath = path
        }
      }
